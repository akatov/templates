...
const projectType: Options.Options<Option.Option<ProjectType>> = Options.all({
  ...
  withESLint,
  withWorkflows,
}).pipe(Options.map(ProjectType.Template), Options.optional);
...
const createProject = Match.type<ResolvedConfig>().pipe(
  Match.when({ projectType: { _tag: "Template" } }, (config) =>
  ...
  return Option.match(config.projectName, {
    ...
    onNone: () =>
      Prompt.text({
        ...
        Effect.flatMap((name) =>
          Path.Path.pipe(Effect.map((path) => path.resolve(name))),
        ),
      ),
  });
}
...
function createTemplate(config: TemplateConfig) {
  return Effect.gen(function* () {
    ...
    if (!config.projectType.withChangesets) {
      ...
      yield* Effect.forEach(patches, (patch) =>
        fs.remove(path.join(config.projectName, "patches", patch)),
      ).pipe(Effect.ignore);
    ...
    if (!config.projectType.withNixFlake) {
      yield* Effect.forEach([".envrc", "flake.nix"], (file) =>
        fs.remove(path.join(config.projectName, file)),
      ).pipe(Effect.ignore);
    }
    ...
    if (config.projectType.template === "devcontainer") {
      filesToCheck.push(
    ...
    yield* Effect.logInfo(
      AnsiDoc.cats([
        ...
        pipe(
          filesToCheck,
          Array.map((file) =>
            AnsiDoc.catWithSpace(AnsiDoc.char("-"), AnsiDoc.text(file)),
          ),
          AnsiDoc.vsep,
          AnsiDoc.indent(2),
        ),
...
const getUserInput = Prompt.select<"template">({
      ...
      case "template": {
        return Prompt.all({
          template: Prompt.select<Template>({
            ...
            choices: [
              {
                title: "Monorepo in Devcontainer",
                value: "devcontainer",
                description: "A monorepo in a devcontainer",
              },
            ],
          }),
